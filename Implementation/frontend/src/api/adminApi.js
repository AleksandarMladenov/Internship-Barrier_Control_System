...
export const inviteAdmin = (payload) =>
  http("/admins/invite", { method: "POST", body: JSON.stringify(payload) }); // returns {id,email,role,status,invite_url}
export const resendInvite = (id) =>
  http(`/admins/${id}/resend-invite`, { method: "POST" });
export const activateAdmin = (id) =>
  http(`/admins/${id}/activate`, { method: "POST" });
export const deactivateAdmin = (id) =>
  http(`/admins/${id}/deactivate`, { method: "POST" });
